...
from NCO_monitor import handle_host_insert, process_report, request_report, retrieve_toggle_list, toggle_module
...
parser.add_argument('--middle_port', type=int,
                    required=False, help="NCO middlebox port")
parser.add_argument('--build_interval', type=int,
                    required=False, help="Construction interval")
parser.add_argument('--query_interval', type=int,
    ...
    try:
        ...
        while True:
            ...
            if host["symvers_ts"] == 0:
                err = request_symver_file(
                    conn, db_connection, host_id, device_mac)
                if err == cfg.DB_ERROR:
            ...
            if len(revoke_id_list) > 0:
                for i in range(len(revoke_id_list)):
                    revoke_module(conn, db_connection, host_id,
                                  revoke_id_list[i], revoke_name_list[i])

            # Revoke requirement: get deprecate list before getting report
            deprecate_id_list = retrieve_deprecated_list(
                db_connection, host_id)
            if len(deprecate_id_list) > 0:
                for i in range(len(deprecate_id_list)):
                    deprecate_module(conn, db_connection,
                                     host_id, deprecate_id_list[i])

            # Monitor requirement: toggle module on/off
            # TODO: what module does this actually fit under?
            toggle_id_list, toggle_mode_list = retrieve_toggle_list(
                db_connection, host_id)
            if len(toggle_id_list) > 0:
                for i in range(len(toggle_id_list)):
                    toggle_module(conn, db_connection,
                                  host_id, toggle_id_list[i], toggle_mode_list[i])
            ...
            if args.challenge:
                ...
                for mod_id in challenge_list:
                    ...
                    if temp == cfg.REVOKE_MOD:
                        ...
                        err = revoke_module(
                            conn, db_connection, host_id, mod_id)
                        if err != 0:
                            logger.info(f"revoke module error")
                    else:
                        # update challenge ts
                        now = int(time.time())
                        update_deployed_sec_ts(
                            db_connection, host_id, mod_id, now)
            ...
            if host == cfg.DB_ERROR:
                ...
                logger.info(
                    "Entering a wait state to interact with host again\n\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        construct = multiprocessing.Process(target=construction_process, name="constuction", args=(
            exit_event, cfg.BUILD_INTERVAL, queue))
        construct.start()
    ...
    try:
        ...
        middlebox = multiprocessing.Process(target=middlebox_process, name="middlebox", args=(
            exit_event, condition, cfg.BUILD_INTERVAL, queue))
        middlebox.start()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            try:
                ...
                device = threading.Thread(target=device_thread, args=(
                    conn, ip, port, cfg.MAX_BUFFER_SIZE, cfg.QUERY_INTERVAL))
                device.start()
