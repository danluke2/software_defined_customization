...
def build_ko_module(db_connection, host_id, module, module_id, key):
    try:
        subprocess.run([cfg.nco_dir + "builder.sh", module, str(module_id),
                       str(cfg.INSERT_LINE), str(host_id), key.hex(), str(cfg.standby), cfg.applyNow], check=True)
        result = 0
...
def construction_loop(db_connection):
    ...
    for i in range(len(host_id_list)):
        key = generate_key()
        err = build_ko_module(
            db_connection, host_id_list[i], module_list[i], cfg.next_module_id, key)
        if err == -1:
