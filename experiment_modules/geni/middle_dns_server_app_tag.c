...
void modify_buffer_recv(struct customization_buffer *recv_buf_st, struct customization_flow *socket_flow)
{
  ...
  //get DNS header and cust tag (will be overwritten)
  copy_success = copy_from_iter_full(recv_buf_st->buf, 12 + cust_tag_test_size, recv_buf_st->src_iter);
  if(copy_success == false)
  {
    trace_printk("L4.5 ALERT: Failed to copy DNS header to cust buffer\n");
    //Scenario 1: keep cust loaded and allow normal msg to be sent
    recv_buf_st->copy_length = 0;
    return;
  }
  recv_buf_st->copy_length = 12 + cust_tag_test_size;


  if(strncmp((char *)recv_buf_st->buf + 12, tag, 5) == 0)
  {
    copy_success = copy_from_iter_full(recv_buf_st->buf + 12, recv_buf_st->recv_return - cust_tag_test_size -12, recv_buf_st->src_iter);
    if(copy_success == false)
    {
      trace_printk("L4.5 ALERT: Failed to copy DNS query cust buffer\n");
      //Scenario 1: keep cust loaded and allow normal msg to be sent
      recv_buf_st->copy_length = 0;
      return;
    }
    recv_buf_st->copy_length = recv_buf_st->recv_return - cust_tag_test_size -12;
