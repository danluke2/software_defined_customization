...
sshpass -p "default" ssh -p 22 root@10.0.0.20 "systemctl stop dnsmasq.service; systemctl stop systemd-resolved.service; dnsmasq --no-daemon -c 0 >/dev/null 2>&1 &"
...
sshpass -p "default" ssh -p 22 root@10.0.0.20 "pkill dnsmasq; $GIT_DIR/DCA_kernel/bash/installer.sh; dnsmasq --no-daemon -c 0 >/dev/null 2>&1 &"
...
sudo rmmod overhead_test_batch_dns_client
sudo rmmod layer4_5

sshpass -p "default" ssh -p 22 root@10.0.0.20 "pkill dnsmasq; rmmod overhead_test_batch_dns_server; rmmod layer4_5; exit"
