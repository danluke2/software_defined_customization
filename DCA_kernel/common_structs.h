...
struct customization_flow
{
  ...
  __be32 source_ip;
  u16 protocol;
  // pid is the thred id and will match thread names like python3, isc-worker,
  // etc.
...
struct customization_socket
{
  ...
  // flag used to signal attached customizations need to be sorted again before
  // use
  bool update_cust_sort;

  // customization must register both send/recv to simplify cust array
  // processing
