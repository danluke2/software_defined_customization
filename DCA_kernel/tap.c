...
int (*ref_tcp_recvmsg)(struct sock* sk,
                       struct msghdr* msg,
                       size_t len,
                       int nonblock,
                       int flags,
...
int (*ref_tcp_v4_connect)(struct sock* sk,
                          struct sockaddr* uaddr,
                          int addr_len);
struct sock* (*ref_inet_csk_accept)(struct sock* sk,
                                    int flags,
                                    int* err,
                                    bool kern);
...
int
new_tcp_sendmsg(struct sock* sk, struct msghdr* msg, size_t size);

#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 0, 0)
int
new_tcp_recvmsg(struct sock* sk,
                struct msghdr* msg,
                size_t len,
                int nonblock,
                int flags,
...
void
new_tcp_close(struct sock* sk, long timeout);
int
new_tcp_v4_connect(struct sock* sk, struct sockaddr* uaddr, int addr_len);
struct sock*
new_inet_csk_accept(struct sock* sk, int flags, int* err, bool kern);

// UDP General reference functions
int (*ref_udp_sendmsg)(struct sock* sk, struct msghdr* msg, size_t size);

#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 0, 0)
int (*ref_udp_recvmsg)(struct sock* sk,
                       struct msghdr* msg,
                       size_t len,
                       int nonblock,
                       int flags,
...
int
new_udp_sendmsg(struct sock* sk, struct msghdr* msg, size_t size);
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 0, 0)
int
new_udp_recvmsg(struct sock* sk,
                struct msghdr* msg,
                size_t len,
                int nonblock,
                int flags,
...
void
new_udp_close(struct sock* sk, long timeout);

extern struct proto tcp_prot;
extern struct proto udp_prot;

void
register_tcp_taps(void)
{
#ifdef DEBUG1
...
// TODO: Consider adding data to the TCP connect to support things like secure
// vector routing (SVR) NOTE: This may be better suited as a BPF function at tc
// or by using BPF paper to wrap in UDP

// Performs a normal TCPv4 connection, but stores the connection for monitoring.
// Customization socket created to allow TCP connections to do during handshake
// phase
...
// TODO: if using mptcp, then this may not be best spot to get port info and
// sock info?
...
// TODO: Would it be useful to send a UDP message to indicate we are closing the
// socket?
               ...
               int (*sendmsg)(struct sock*, struct msghdr*, size_t))
{
...
#ifdef DEBUG3
    trace_printk(
      "L4.5: NULL cust socket in sendmsg, creating cust socket for pid %d\n",
      task->pid);
#endif
  ...
  // if update check set due to new cust module, then need to check if socket
  // should be customized now
...
// TODO: Is it better to call common_sendmsg without passing ref_xxx_sendmsg as
// a param?
//  Would return to new_xxx_sendmsg instead, which could either send normal or
//  call the dca Also, should dca return here before sending?  This may be
//  complicated though
...
new_tcp_sendmsg(struct sock* sk, struct msghdr* msg, size_t size)
{
  ...
  // task->pid, task->tgid, task->comm); trace_printk("L4.5 TCP sendmsg, real
  // parent pid %d, tgid %d, task name = %s\n", task_rparent->pid,
  // task_rparent->tgid, task_rparent->comm); trace_printk("L4.5 TCP sendmsg,
  // parent pid %d, tgid %d, task name = %s\n", task_parent->pid,
  // task_parent->tgid, task_parent->comm);
  //
  // if(test)
  // {
...
new_udp_sendmsg(struct sock* sk, struct msghdr* msg, size_t size)
{
  ...
  // task->pid, task->tgid, task->comm); trace_printk("L4.5 UDP sendmsg, real
  // parent pid %d, tgid %d, task name = %s\n", task_rparent->pid,
  // task_rparent->tgid, task_rparent->comm); trace_printk("L4.5 UDP sendmsg,
  // parent pid %d, tgid %d, task name = %s\n", task_parent->pid,
  // task_parent->tgid, task_parent->comm);
  //
  // if(test)
...
#else
    return recvmsg(sk, msg, len, flags, addr_len);
#endif
  }

// NOTE: This is used for targeting a specific app during testing, so probably
// better as a new debug type
#ifdef APP
...
#else
  recvmsg_return = recvmsg(sk, msg, len, flags, addr_len);
#endif
  // TODO: Should we let these messages go through cust module in case it wants
  // to add data?
  if (recvmsg_return <= 0) {
// a 0 length or error message has no need for customization removal
#ifdef DEBUG1
    trace_printk(
      "L4.5: received %d length message, pid %d\n", recvmsg_return, task->pid);
#endif
...
#ifdef DEBUG3
    trace_printk(
      "L4.5: NULL cust socket in recvmsg, creating cust socket for pid %d\n",
      task->pid);
#endif
                ...
                int* addr_len)
{
  ...
  // task->pid, task->tgid, task->comm); trace_printk("L4.5 TCP recvmsg, real
  // parent pid %d, tgid %d, task name = %s\n", task_rparent->pid,
  // task_rparent->tgid, task_rparent->comm); trace_printk("L4.5 TCP recvmsg,
  // parent pid %d, tgid %d, task name = %s\n", task_parent->pid,
  // task_parent->tgid, task_parent->comm); if(test)
  // {
  //   trace_printk("L4.5 TCP recvmsg, test pid %d, tgid %d, task name = %s\n",
  //   test->pid, test->tgid, test->comm);
  // }
  return common_recvmsg(
    sk, msg, len, nonblock, flags, addr_len, ref_tcp_recvmsg);
}
#else
int
new_tcp_recvmsg(struct sock* sk,
                ...
                int* addr_len)
{
  ...
  //   test->pid, test->tgid, test->comm);
  // }
  return common_recvmsg(sk, msg, len, flags, addr_len, ref_tcp_recvmsg);
}
#endif
...
{
  return common_recvmsg(
    sk, msg, len, nonblock, flags, addr_len, ref_udp_recvmsg);
}
#else
int
new_udp_recvmsg(struct sock* sk,
                ...
                int* addr_len)
{
  return common_recvmsg(sk, msg, len, flags, addr_len, ref_udp_recvmsg);
}
#endif
