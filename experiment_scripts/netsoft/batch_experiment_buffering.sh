...
OUTPUT=$EXP_SCRIPT_DIR/logs/batch_buffer_base.txt
touch $OUTPUT
...
OUTPUT=$EXP_SCRIPT_DIR/logs/batch_buffer_tap.txt
touch $OUTPUT
...
OUTPUT=$EXP_SCRIPT_DIR/logs/batch_buffer_cust.txt
touch $OUTPUT

sshpass -p "$SERVER_PASSWD" ssh -p 22 root@$SERVER_IP "pkill dnsmasq; cd $NETSOFT_MOD_DIR; make BUILD_MODULE=overhead_test_batch_dns_server_buffer.o; insmod overhead_test_batch_dns_server_buffer.ko; dnsmasq --no-daemon -c 0 >/dev/null 2>&1 &"
...
python3 batch_plot_buffering.py
