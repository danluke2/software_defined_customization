...
dns_data_tap = []
with open("../logs/batch_buffer_tap.txt") as fp:
    while True:
        time = fp.readline()
        if not time:
            break
...
dns_data_cust = []
with open("../logs/batch_buffer_cust.txt") as fp:
    while True:
        time = fp.readline()
        if not time:
            break
...
dns_data = [dns_data_tap, dns_data_cust]
...
plt.xticks([1, 2], ["L4.5 Tap", "L4.5 Tap+Cust"], rotation=0)
plt.ylabel('Seconds')
plt.title("DNS Batch Query/Response Time with L4.5 Buffering")

# plt.show()
plt.savefig('batch_buffered_overhead.png')
