...
OUTPUT=$EXP_SCRIPT_DIR/logs/deprecate_exp.txt
touch $OUTPUT
...
python3 $NCO_DIR/deploy_module_helper.py --module "demo_dns_server_app_tag" --host 2 --priority 42
...
do
  ...
  for ((j=1;j<=$2;j++))
  do
    query="www.test_deprecate$i$j.com"
    before=$(date '+%s%6N');
    dig @$SERVER_IP -p 53 $query > /dev/null;
    after=$(date '+%s%6N');
    total=$((total+(after-before)));
