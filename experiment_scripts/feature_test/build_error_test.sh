...
# Purpose: try to build a module that has an error in it and make sure tables are updated
...
echo "*************** Starting NCO  ***************"
gnome-terminal -- bash -c "echo '*************** Starting NCO  ***************';  python3 $NCO_DIR/NCO.py --build_interval 3 --query_interval 5 --linear --print --logfile $NCO_DIR/build_error_messages.log"

sleep 2

# start DCA process, which will have host_id = 1
echo "*************** Starting DCA  ***************"
gnome-terminal -- bash -c "echo '*************** Starting DCA  ***************';  python3 $DCA_USER_DIR/DCA.py --print --logfile $DCA_USER_DIR/build_error_messages.log"

sleep 5

# Insert challenge module in DB for deployment to host_id = 1
echo "*************** Deploy Module  ***************"
python3 $NCO_DIR/deploy_module_helper.py --module "error_module" --host 1

sleep 10

# save tracelog files from each host
sudo cp /sys/kernel/tracing/trace $EXP_SCRIPT_DIR/logs/build_error.txt
