...
gnome-terminal -- bash -c "echo '*************** Starting NCO  ***************'; python3 $NCO_DIR/NCO.py --query_interval 5 --build_interval 3 --linear --print --ip $CLIENT_IP --logfile $NCO_DIR/priority_stream_single_2_nco_messages.log"

sleep 3

# start client DCA process, which will have host_id = 1
echo "*************** Starting DCA on Client  ***************"
gnome-terminal -- bash -c "echo '*************** Client DCA  ***************'; python3 $DCA_USER_DIR/DCA.py --print --logfile $DCA_USER_DIR/priority_stream_single_2_client_dca_messages.log --ip $CLIENT_IP"

sleep 5

# start DCA process on server, which will have host_id = 2
echo "*************** Starting DCA on Server  ***************"
sshpass -p "$SERVER_PASSWD" ssh -p 22 root@$SERVER_IP "python3 $DCA_USER_DIR/DCA.py --ip $CLIENT_IP --logfile $DCA_USER_DIR/priority_stream_single_2_server_dca_messages.log  >/dev/null 2>&1 &"

sleep 5

# Insert server module in DB for deployment to host_id = 2, with active mode set to true and with applyNow set
echo "*************** Deploy Server Module 30s  ***************"
python3 $NCO_DIR/deploy_module_helper.py --module "rate_limit__30_sec" --host 2 --priority 10 --applyNow --activate

sleep 7

# insert lower pri module
echo "*************** Deploy Server Module 100msec  ***************"
...
gnome-terminal -- bash -c "echo '*************** Starting TCPDUMP  ***************'; tcpdump port 8080 -i any -w $GIT_DIR/priority_stream_single_2.pcap"

sleep 10

echo "*************** starting 30sec delay download ***************"
...
echo "*************** Revoke Server 100msec Module ***************"
python3 $NCO_DIR/revoke_module_helper.py --module "activate_web_server" --host 2

# Revoke server 1000msec module in DB host_id = 2
echo "*************** Revoke Server 30s Module ***************"
python3 $NCO_DIR/revoke_module_helper.py --module "rate_limit_30_sec" --host 2
...
sudo cp /sys/kernel/tracing/trace $EXP_SCRIPT_DIR/logs/priority_stream_single_2_client.txt

rmmod layer4_5
pkill tcpdump

sshpass -p "$SERVER_PASSWD" ssh -p 22 root@$SERVER_IP "pkill python3; sudo cp /sys/kernel/tracing/trace $EXP_SCRIPT_DIR/logs/priority_stream_single_2_server.txt; rmmod layer4_5"
