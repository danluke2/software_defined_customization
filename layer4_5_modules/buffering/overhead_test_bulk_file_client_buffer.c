...
void modify_buffer_recv(struct customization_buffer *recv_buf_st, struct customization_flow *socket_flow)
{
    ...
    // printk(KERN_INFO
    //        "L4.5: Total bytes from server to cust mod = %lu, insert index = %lu, recvmsg=%lu, req_length=%lu\n",
    //        total_bytes_from_server, insert_index, recv_buf_st->recv_return, recv_buf_st->length);
    // trace_printk("L4.5: Total bytes from server to cust mod = %lu, insert index = %lu, recvmsg=%lu,
    // req_length=%lu\n",
    //              total_bytes_from_server, insert_index, recv_buf_st->recv_return, recv_buf_st->length);
    ...
    // trace_printk("L4.5 Start Params: recvmsg=%lu, tag_bytes=%lu, extra_bytes=%lu, total_bytes=%lu, length=%lu, "
    //              "insert_index=%lu\n",
    ...
    if (tag_bytes_removed_last_round > 0)
    {
        ...
        {
            trace_printk("L4.5: Hit edge case, just tag bytes left in packet\n");
            goto buffer_and_send;
        }
    }
...
buffer_and_send:
    ...
    if (insert_index > recv_buf_st->length)
    {
        ...
        // trace_printk("L4.5: insert index larger, index = %lu\n", insert_index);
    ...
    else
    {
        ...
        // trace_printk("L4.5: insert index smaller, index = %lu\n", insert_index);
        // app can take it all
