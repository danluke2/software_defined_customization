...
void modify_buffer_recv(struct customization_buffer *recv_buf_st, struct customization_flow *socket_flow)
{
    ...
    {
        if (strncmp(cust_tag_test, recv_buf_st->src_iter->iov->iov_base, cust_tag_test_size) == 0)
        {
            iov_iter_advance(recv_buf_st->src_iter, cust_tag_test_size);
            copy_success = copy_from_iter_full(recv_buf_st->buf, recv_buf_st->recv_return - cust_tag_test_size,
            ...
            // trace_print_hex_dump("DNS packet recv: ", DUMP_PREFIX_ADDRESS, 16, 1, recv_buf_st->buf, *copy_length,
            // true);
        }
        else
        {
