...
void modify_buffer_recv(struct customization_buffer *recv_buf_st, struct customization_flow *socket_flow)
{
    ...
    // Leaving off the negative 1 to make the math right, not sure why
    size_t cust_test_size2 = (size_t)sizeof(cust_test2);
    ...
    // Mine
    recv_buf_st->copy_length = recv_buf_st->recv_return - cust_test_size - cust_test_size2;
    ...
    //memcpy(send_buf_st->buf + cust_test_size + send_buf_st->length, cust_test2, cust_test_size2);
    // End from the client send

    // memcpy(recv_buf_st->buf,)
