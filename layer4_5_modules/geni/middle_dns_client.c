...
// insert tag after 12 byte DNS header and before query
void modify_buffer_send(struct customization_buffer *send_buf_st, struct customization_flow *socket_flow)
{
    ...
    // copy header only
    copy_success = copy_from_iter_full(send_buf_st->buf, 12, send_buf_st->src_iter);
    if (copy_success == false)
    {
        trace_printk("L4.5 ALERT: Failed to copy DNS header into buffer\n");
        return;
    }
    send_buf_st->copy_length += 12;

    memcpy(send_buf_st->buf + 12, cust_tag_test, cust_tag_test_size);
    send_buf_st->copy_length += cust_tag_test_size;

    copy_success = copy_from_iter_full(send_buf_st->buf + 12 + cust_tag_test_size, send_buf_st->length - 12,
                                       send_buf_st->src_iter);
    if (copy_success == false)
    {
        trace_printk("L4.5 ALERT: Failed to copy DNS query into buffer\n");
        return;
    }

    send_buf_st->copy_length += send_buf_st->length - 12;
    return;
}
...
int __init sample_client_start(void)
{
    ...
    dns_cust->send_buffer_size = 4096; // accept default buffer size
