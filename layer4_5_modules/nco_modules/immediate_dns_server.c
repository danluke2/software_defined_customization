...
char cust_tag_test[33] = "xxxxxxxDNSxCLEARANCExCODExxxxxxx";
...
void modify_buffer_recv(struct customization_buffer *recv_buf_st, struct customization_flow *socket_flow)
{
    ...
    if (strncmp((char *)recv_buf_st->src_iter->iov->iov_base, cust_tag_test, cust_tag_test_size) == 0)
    {
    ...
    {
        // something came in without proper clearance code
        trace_printk("L4.5 ALERT: DNS packet does not match requirements, size = %lu\n", recv_buf_st->recv_return);
        recv_buf_st->copy_length = 0;
    }
